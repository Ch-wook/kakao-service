              ...
              {!isCollapsed && (
                <div>
                  {group.members.map((member) => {
                    ...
                    return (
                      <div key={member.id} className="flex items-center gap-2 px-4 py-1.5">
                        <button
                          onClick={() => onToggleStatus(widget.id, group.id, member.id)}
                          className={`flex-none px-2 py-0.5 rounded-md text-xs font-semibold transition-colors active:opacity-70 ${cfg.bgColor} ${cfg.textColor}`}
                        >
                          {cfg.label}
                        </button>
                        <div className="flex-1 min-w-0">
                          {editingMember?.groupId === group.id && editingMember.memberId === member.id ? (
                            <input
                              ...
                              autoFocus
                              className="w-full text-sm px-2 py-0.5 border border-blue-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 bg-blue-50"
                            />
                          ) : (
                            <span
                              className="text-sm text-gray-800 cursor-pointer hover:text-blue-600 block truncate leading-tight"
