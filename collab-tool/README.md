...
> **노션은 "준비된 팀"을 위한 도구, Collab은 "지금 당장 움직이는 사람들"을 위한 도구**

카톡방에서 "노션 링크 보낼게요" 하면 — 앱 설치, 회원가입, 초대 수락, 결국 카톡으로 다시 소통.  
Collab은 이 마찰을 **0**으로 만듭니다.

| 항목 | 노션 | Collab |
|------|------|--------|
| 진입 방법 | 회원가입 → 로그인 → 초대 수락 | 링크 탭 → 닉네임 입력 → 즉시 참여 |
| 대상 | 장기 프로젝트, 문서 관리 | 일회성 이벤트, 단발 모임 |
| UI 복잡도 | 학습 필요 | 버튼 몇 개로 끝 |
| 맥락 | PC 중심 업무 | 카톡방 → 링크 탭 → 폰에서 바로 사용 |
| 목적 | 지식 저장, 문서화 | 지금 이 순간 상황 공유 |
