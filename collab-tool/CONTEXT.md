...
| 15단계 | 이미지 갤러리 위젯 (Supabase Storage 업로드, 3컬럼 그리드, 라이트박스, 다중 업로드) | ✅ |
| 16단계 | 음악 플레이어 위젯 (Supabase Storage 업로드, 재생목록, HTML5 Audio, 트랙명 수정) | ✅ |
| 17단계 | UX 개선: 위젯 추가 칩 바, 공지 하단 오버레이, 음악 XHR progress 업로드 | ✅ |
| 18단계 | 탭 바 통합 (단일 스크롤) + 위젯 타입별 필터링, 음악 업로드 JWS 오류 수정 | ✅ |
| 배포 | Vercel 배포 완료 | ✅ |
...
└── collab-tool/
    ...
    │       ├── ImageGalleryWidget.tsx # 이미지 갤러리 (Supabase Storage, 라이트박스)
    │       ├── MusicPlayerWidget.tsx  # 음악 플레이어 (HTML5 Audio, 재생목록)
...
### Storage 설정 (15·16단계 추가 시 필요)

Supabase 대시보드 → Storage → 버킷 생성: `collab-files` (Public ON)

```sql
-- Storage 정책 (SQL Editor에서 실행)
CREATE POLICY "Anyone can upload" ON storage.objects
  FOR INSERT WITH CHECK (bucket_id = 'collab-files');
CREATE POLICY "Anyone can view" ON storage.objects
  FOR SELECT USING (bucket_id = 'collab-files');
CREATE POLICY "Anyone can delete" ON storage.objects
  FOR DELETE USING (bucket_id = 'collab-files');
```

파일 경로 패턴: `images/{room_id}/{widget_id}/{uuid}-{filename}` / `music/{room_id}/{widget_id}/{uuid}-{filename}`
...
// 이미지 갤러리
{
  images: [{
    ...
    filename: string, uploaderNickname?: string,
...
// 음악 플레이어
{
  tracks: [{
    id: string, url: string, storagePath: string,
    name: string, originalFilename: string,
    uploaderNickname?: string, uploadedAt: string, size: number
  }]
}
...
// 이미지 갤러리
uploadImage(widgetId, file, nickname, onProgress)  // Supabase Storage 업로드, Optimistic
deleteImage(widgetId, imageId)                     // Storage + DB 동시 삭제

// 음악 플레이어
uploadTrack(widgetId, file, nickname, onProgress)  // Supabase Storage 업로드, Optimistic
deleteTrack(widgetId, trackId)                     // Storage + DB 동시 삭제
updateTrackName(widgetId, trackId, newName)        // Optimistic
```
