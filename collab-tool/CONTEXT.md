...
| 8단계 | 정산 납부 현황 강화, 회계 장부 탭, 납부 체크 위젯, 멤버 메모 | ✅ |
...
└── collab-tool/
    ...
    │   └── Widgets/
    │       ├── ChecklistWidget.tsx
    │       ├── ExpenseWidget.tsx     # 정산(N빵) + paidAmount + 납부 현황 요약
    │       ├── MemberWidget.tsx      # 멤버 관리 + 멤버별 메모(note) 필드
    │       ├── LedgerWidget.tsx      # 회계 장부 (장부 탭 전용, 삭제 불가)
    │       ├── FeeWidget.tsx         # 납부 체크 위젯 (개인별 등록비 관리)
...
-- 최신 위젯 타입 constraint (ledger, fee 포함)
ALTER TABLE widgets DROP CONSTRAINT widgets_type_check;
ALTER TABLE widgets ADD CONSTRAINT widgets_type_check
  CHECK (type IN ('checklist','expense','member','ledger','fee','vote','memo','schedule','roles','poll'));
...
// 정산 (N빵)
{
  totalAmount: number,
  description: string,
  payers: [{ name, paid, paidAmount? }]  // paidAmount: 실제 납부 금액 (없으면 1인당 금액)
}
...
// 멤버 관리
{
  groups: [{
    ...
    name: string,
    targetCount?: number,
    members: [{ id, name, status: 'unknown'|'attending'|'arrived'|'preparing'|'absent'|'home', note?: string }]
    // note: 메모/도착시간 (예: '9:30 도착', '오는 중')
  }]
}

// 회계 장부 (장부 탭 전용, 삭제 불가, 자동 생성)
{
  entries: [{
    id: string,
    date: string,        // YYYY-MM-DD
    category: string,    // '식비'|'교통비'|'숙박비'|'입장료'|'쇼핑'|'기타'|직접입력
    description: string,
    amount: number,
    type: 'expense'|'income'
  }]
...
// 납부 체크
{
  defaultAmount: number,  // 기본 1인 납부 금액
  entries: [{
    id: string,
    name: string,         // 납부자 이름
    amount?: number,      // 개별 금액 (없으면 defaultAmount 사용)
    paid: boolean,
    note?: string         // 비고 (예: '현금 납부', '분할 납부 예정')
  }]
}
```
...
정산(ExpenseWidget), 납부 체크(FeeWidget), 멤버 관리(MemberWidget) 세 위젯 모두 이름 입력란에서 **쉼표(,)** 로 여러 이름을 한 번에 추가할 수 있습니다.

```
홍길동,김철수,이영희  →  Enter  →  3명 동시 추가
```

- 쉼표 앞뒤 공백은 자동 trim
- 이미 목록에 있는 이름은 자동 제외 (중복 방지)
- FeeWidget 상세 폼에서 여러 명 일괄 추가 시 비고(note)는 미적용 (1명일 때만 의미 있음)
- 구현 위치: 각 위젯의 `addPayer` / `handleAddEntry` / `handleAddMember` 함수에서 `.split(',')` 처리
...
togglePayerStatus(widgetId, payerName)            // Optimistic, paidAmount 자동 설정
...
// 회계 장부
updateLedgerData(widgetId, data: LedgerData)     // Optimistic

// 납부 체크
updateFeeData(widgetId, data: FeeData)           // Optimistic
toggleFeeEntry(widgetId, entryId)                // Optimistic
```
...
### 회계 장부 보호 전략
- ledger 타입 위젯은 `displayWidgets`에서 필터링 → 위젯 탭에 표시 안 됨
- 장부 탭 첫 진입 시 `createWidget('ledger', '회계 장부')` 자동 호출
- `onDeleteWidget` prop 전달 안 함 → 삭제 버튼 없음
- `ledgerInitRef`로 중복 생성 방지
...
| API validTypes에 member 누락 | `api/widgets/route.ts` | member, ledger, fee 추가 |
| 이름 일괄 입력 구분자 오작동 | ExpenseWidget / FeeWidget / MemberWidget | 공백 구분 제거, 쉼표(,)만 인식 |
